		...
		do {
			...
			if (verificationFile != null && decodedSymbol.getNumber()>=0) {
				if (decodedSymbol.getChar() != this.verificationFile.charAt(verificationFileIndex)) {
					throw new AritmethicCompressionException("Se esperaba " + new Symbol(this.verificationFile.charAt(verificationFileIndex)) + " pero vino " + decodedSymbol + " en la posición " + verificationFileIndex);
				}
			}
